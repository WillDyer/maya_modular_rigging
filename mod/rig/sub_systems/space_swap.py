...
class SpaceSwapping():
    def __init__(self, key):
        ...
        if rig_type == "FKIK":
            ...
            if self.system_to_be_made["ik_ctrl_list"]:
                ...
                self.handle_ctrl = [x for x in self.ik_ctrls if cmds.attributeQuery("handle", node=x, exists=True)
                                    and cmds.getAttr(f"{x}.handle", asString=1) == "True"]
    ...
    def create_locators(self):
        ...
        if "Custom" in self.locator_list:
            loc = cmds.spaceLocator(n="loc_space_custom_#")[0]
